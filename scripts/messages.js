/****************************************************
 * CONTACT.COM — PRODUCTION MESSAGES.JS (ECHO FIXED)
 ****************************************************/
...
async function loadMessagesOnce(showSpinner = true) {
  ...
  try {
    ...
    serverMessages.forEach(msg => {
      ...
      if (!renderedMessageIds.has(uniqueKey) && !renderedMessageIds.has(optimisticKey)) {
        fragment.appendChild(buildMessageRow(msg));
...
function sendMessage(payloadOverride = null) {
  ...
  // THE FIX: Create a more specific lock key to match polling logic
  const lockKey = `sent_${payload.text || payload.fileName}_${timestamp}`;
